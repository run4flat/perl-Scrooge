...
*prs = \&Scrooge::PDL::parse_range_strings;
...
$two_to_five = re_intersect(name => 'test regex', above => -1, below => 1, quantifiers => [1, 200]);
($matched, $offset) = $two_to_five->apply($data);
isnt($matched, undef, 'two_to_five matched piddle');
is($matched, 100, 'whole segment matched');
is($offset, 0, 'identifies first matching element');

$two_to_five = re_intersect(name => 'test regex', above => -1, below => 1, quantifiers => [50, 200]);
($matched, $offset) = $two_to_five->apply($data);
isnt($matched, undef, 'two_to_five matched piddle');
is($matched, 100, 'whole segment matched');
is($offset, 0, 'identifies first matching element');